...
class _StatusPageState extends ConsumerState<StatusPage> {
  ...
  Future<void> _onToggleOfferExitForAll(bool enabled) async {
    ...
    final peers = ref.read(knownPeersProvider).valueOrNull ?? const <KnownPeer>[];
    if (peers.isEmpty) return;

    setState(() => _offerAllBusy = true);
    try {
      final api = ref.read(apiProvider);
      final futures = <Future<String>>[];
      for (final p in peers) {
        futures.add(() async {
          final cfg = await api.fetchKnownPeerConfig(p.peerID);
          ...
          return api.updateKnownPeerConfig(payload);
        }());
      }

      final results = await Future.wait(futures);
      final firstErr = results.firstWhere((e) => e.isNotEmpty, orElse: () => '');
      if (firstErr.isNotEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(backgroundColor: Theme.of(context).colorScheme.error, content: Text(firstErr)),
          );
        }
        return;
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(enabled ? 'Exit node enabled for all peers' : 'Exit node disabled for all peers')),
        );
      }
    } finally {
...
class _OfferExitForAllRow extends StatelessWidget {
  final List<KnownPeer>? knownPeers;
  final bool busy;
  final Future<void> Function(bool enabled)? onToggle;

  const _OfferExitForAllRow({required this.knownPeers, required this.busy, required this.onToggle});

  @override
  Widget build(BuildContext context) {
    final colorScheme = Theme.of(context).colorScheme;
    final peers = knownPeers ?? const <KnownPeer>[];

    final enabledCount = peers.where((p) => p.weAllowUsingAsExitNode).length;
    final allEnabled = peers.isNotEmpty && enabledCount == peers.length;
      ...
      children: [
        Expanded(
          child: Column(
            ...
            children: [
              Text('Activate exit node', style: Theme.of(context).textTheme.bodyLarge?.copyWith(fontWeight: FontWeight.w500)),
        ...
        Switch(
          value: allEnabled,
          onChanged: (busy || onToggle == null || peers.isEmpty) ? null : (val) => onToggle!(val),
        ),
      ],
    );
  }
        ...
        return SafeArea(
          child: Column(
            ...
            children: [
              ...
              Column(
                mainAxisSize: MainAxisSize.min,
                children: names.map((n) => RadioListTile<String>(
                  title: Text(n),
                  value: n,
                  groupValue: selected,
