    ...
    return ValueListenableBuilder<bool>(
      ...
      builder: (context, isAvailable, child) {
        ...
        return PeersListView(
          ...
          onSetExitPeer: (usingPeerID) async {
            ...
            if (response == "") {
              ...
              if (context.mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(backgroundColor: Theme.of(context).colorScheme.error, content: Text(response)),
                );
              }
            }
          },
          onToggleOfferAsExit: (peer, enabled) async {
            // Need full peer config to update, because backend expects alias/domain/ipAddr too.
            final cfg = await ref.read(apiProvider).fetchKnownPeerConfig(peer.peerID);
            final payload = UpdateKnownPeerConfigRequest(
              cfg.peerId,
              cfg.alias,
              cfg.domainName,
              cfg.ipAddr,
            ...
            final response = await ref.read(apiProvider).updateKnownPeerConfig(payload);
            if (response == "") {
              await Future.wait([
                ref.read(knownPeersProvider.notifier).refresh(),
                ref.read(availableProxiesProvider.notifier).refresh(),
              ]);
            } else {
              if (context.mounted) {
...
class PeersListView extends StatefulWidget {
  ...
  final Future<void> Function(KnownPeer peer, bool enabled)? onToggleOfferAsExit;
  final Future<void> Function(String usingPeerID)? onSetExitPeer;
...
class _PeersListViewState extends State<PeersListView> {
  ...
  final Map<String, bool> _busyOfferExit = {};
  final Map<String, bool> _busyUseExit = {};
  ...
  Widget _buildExpansionPanelBody(KnownPeer item) {
    ...
    if (item.connections.isNotEmpty) {
      connectionWidget = _buildConnectionsWidget(item.connections);
    }

    // --- Exit node controls ---

    Widget useAsExitControl() {
      final isCurrent =
          widget.proxyExitPeerID != null && widget.proxyExitPeerID!.isNotEmpty && widget.proxyExitPeerID == item.peerID;
      final canUse = item.allowedUsingAsExitNode;
      final busy = _busyUseExit[item.peerID] ?? false;

      final label = isCurrent ? 'Stop using' : 'Use';

      final btn = FilledButton.tonal(
        onPressed: (!canUse || busy || widget.onSetExitPeer == null)
            ? null
            : () async {
                setState(() => _busyUseExit[item.peerID] = true);
                try {
                  await widget.onSetExitPeer!(isCurrent ? '' : item.peerID);
                } finally {
                  if (mounted) setState(() => _busyUseExit[item.peerID] = false);
                }
              },
        child: busy
      ...
      if (canUse) return btn;
      return Tooltip(message: 'This peer has not offered itself as an exit node for you.', child: btn);
    }

    Widget offerAsExitControl() {
      final busy = _busyOfferExit[item.peerID] ?? false;
      ...
      return Row(
        ...
        children: [
          ...
          Switch(
            value: item.weAllowUsingAsExitNode,
            onChanged: (busy || widget.onToggleOfferAsExit == null)
                ? null
                : (val) async {
                    setState(() => _busyOfferExit[item.peerID] = true);
                    try {
                      await widget.onToggleOfferAsExit!(item, val);
                    } finally {
                      if (mounted) setState(() => _busyOfferExit[item.peerID] = false);
                    }
                  },
          ),
        ],
    ...
    if (isWide) {
      ...
      if (connectionWidget != null) {
        ...
        cells.insert(insertIdx, _buildGridCell("CONNECTION", connectionWidget, colorScheme));
      }

      // Exit-node controls
      cells.add(_buildGridCell("USE AS EXIT", useAsExitControl(), colorScheme));
      cells.add(_buildGridCell("OFFER AS EXIT", offerAsExitControl(), colorScheme));
      ...
      rows.add(_buildMobileRow("USE AS EXIT", useAsExitControl()));
      rows.add(_buildMobileRow("OFFER AS EXIT", offerAsExitControl()));
